...
仕事（Work）と私生活（Life）、2つの独立したGoogleカレンダーを相互接続（Peering）し、 *「ワークライフバランス」* をサポートする Google Apps Script (GAS) ツールです。

サーバー不要、完全無料で動作し、プライバシーに配慮して設計思想です。
...
これは、 *互いの領域の独立性を保ったまま、「公開してもよい予定」と「公開したくない予定」を、自分のルールで相互に広報（Advertise）するので「ピアリング」* と名乗りました。

いわば、 *「自分の時間を守るための、双方のワガママ（Opinionated）な要求をいい感じにピアリングするルータ」* として機能します。
...
| `HOLIDAY_IGNORE_LIST` | 祝日判定から除外するイベント | `節分,バレンタイン,雛祭り,母の日,父の日,七夕,ハロウィン,クリスマス`<br>※「日本の祝日」カレンダーが平日の行事も掲載しているための対策 |
