# Google Calendar Peering

仕事（Work）と私生活（Life）、2つの独立したGoogleカレンダーを相互接続（Peering）し、 *「ワークライフバランス」* をサポートする Google Apps Script (GAS) です。

サーバー不要、完全無料で動作し、プライバシーに配慮した設計思想に基づき実装しています。

## 🎯 コンセプト - Why "Peering"?

本ツールが「Sync（同期）」ではなく **「Peering（ピアリング）」** を名乗るのには理由があります。
単に予定をコピーして共有するだけの「受動的な同期」ではありません。
これは 互いの領域の独立性を尊重し「共有してよい予定」と「公開したくない予定」を「能動的に同期」するので **「Peering(ピアリング)」** と名乗りました。

いわば *「双方のワガママ（Opinionated）な要求をいい感じにピアリングするルータ」* として機能します。

* **Work → Life**
    * **共有内容:** 家族やパートナー、あるいは自分自身のために確保できる「休日・祝日」の情報。
    * **目的:** 仕事の予定で埋まりがちなカレンダーの中に、**私生活（Life）のための「空白」を明確化**し、プライベートな計画を立てやすくします。

* **Life → Work**
    * **共有内容:** 通院、役所手続き、あるいは「絶対に邪魔されたくない趣味の時間」など、「私生活」の情報。
    * **目的:** 具体的な内容は隠蔽（マスク）したまま「休暇」として共有し、**職場からの会議招待を回避**します。

## 🚀 特徴

* **Work-Life Balance First:** 「仕事」と「人生」を対等な関係として扱います。
* **完全無料 & サーバーレス:** Google Apps Script (GAS) だけで完結し、常時稼働サーバーは不要です。
* **Work -> Life (自動共有):** 休日・祝日の仕事予定、および平日の勤務時間外（始業前・終業後）の予定を自動でLifeカレンダーへ連携します。
...
### 1. プロジェクトの作成
Google Apps Script プロジェクトを新規作成。`clasp` 好きならローカルに `clasp clone` してください。

### 2. コードのデプロイ
`Code.js` の内容をコピー＆ペーストするか、`clasp push` してください。
...
| `WORK_START_HOUR` | 勤務開始時刻（0〜23の整数）。この時刻より前の平日WORKイベントをLIFEに自動同期 | `10` |
| `WORK_END_HOUR` | 勤務終了時刻（1〜23の整数）。この時刻以降の平日WORKイベントをLIFEに自動同期 | `18` |
...
| Work: 平日の勤務時間外（始業前・終業後）の予定 | → Life（自動、そのまま表示） |
...
- GASからのアクセス確認するために testAccess() を用意しています。読み取り権限のみの確認です。単独で実行してください。
